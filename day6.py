...
'''
def traverse_graph(orbit_map):
    running_total = 0
    orbital_stack = orbit_map
    cache = dict({'COM': 0})

    for orbital in orbital_stack:
        orbital_start, orbital_end = orbital.split(')')
        if orbital_start in cache:
            cache[orbital_end] = cache[orbital_start] + 1
            running_total = running_total + cache[orbital_start] + 1
        else:
            orbital_stack.append(orbital)
    return running_total

assert(traverse_graph(['COM)B','B)C','C)D','D)E','E)F','B)G','G)H','D)I','E)J','J)K','K)L']) == 42)
assert(traverse_graph(['E)J', 'COM)B','B)C','C)D','D)E','E)F','B)G','G)H','D)I','J)K','K)L']) == 42)

print(traverse_graph(new_input.copy()))
...
def least_moves(orbit_map):
    orbital_stack = orbit_map
    cache = dict({'COM': []})

    for orbital in orbital_stack:
        if 'SAN' in cache and 'YOU' in cache:
            break
        orbital_start, orbital_end = orbital.split(')')
        if orbital_start in cache:
            cache[orbital_end] = cache[orbital_start] + [orbital_end]
        else:
            orbital_stack.append(orbital)

    for index, orbit in enumerate(cache['YOU'][::-1]):
        if orbit in cache['SAN']:
            return(index + cache['SAN'][::-1].index(orbit) - 2)

assert(least_moves(['COM)B', 'B)C', 'C)D', 'D)E', 'E)F', 'B)G', 'G)H', 'D)I', 'E)J', 'J)K', 'K)L', 'K)YOU', 'I)SAN']) == 4)
assert(least_moves(['COM)B', 'B)C', 'C)D', 'D)E', 'B)G', 'G)H', 'D)I', 'E)J', 'J)K', 'K)L', 'K)YOU', 'I)SAN', 'E)F']) == 4)
print(least_moves(new_input.copy()))
